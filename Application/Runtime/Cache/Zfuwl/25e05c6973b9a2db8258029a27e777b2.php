...
<div class="admin-main">
    <fieldset class="layui-elem-field">
        ...
        <form action="<?php echo U('Excel/exportPayLog');?>" id="search-form2">
            ...
            <div class="layui-inline">
                <div class="layui-input-inline">
                    <select name="bank_id" class="layui-input">
                        <option value="">--汇入银行--</option>
                        <?php if(is_array($bankInfo)): foreach($bankInfo as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="layui-inline">
                <div class="layui-input-inline">
                    <select name="type" class="layui-input">
                        ...
                        <?php if(is_array($addMoneyType)): foreach($addMoneyType as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="layui-inline"><div class="layui-input-inline"><input type="text" name="time" placeholder="充值时间" id="time" class="layui-input" /></div></div>
            ...
            <button type="button" class="clearCache layui-btn  layui-btn-danger pull-right" onclick="emptyEp();"><i class="layui-icon">&#xe60b;</i> 清空记录</button>
...
<script>
    ...
    function ajax_get_table(tab, page){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo U('Recharge/addUserMoney');?>?p=" + page,
            data: $('#' + tab).serialize(),
    ...
    function emptyEp(){
        layer.confirm("是否确认清空数据，执行之后将不能撤销", {title:'温馨提示', icon:3}, function(){
            ...
            var url = "<?php echo U('Recharge/emptyAddUserMoneyLog');?>";
