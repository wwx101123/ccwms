...
<div class="admin-main">
    <fieldset class="layui-elem-field">
        <blockquote class="layui-elem-quote">
            <form action="<?php echo U('Excel/exportBonusLog');?>" id="search-form2">
                ...
                <div class="layui-inline"><div class="layui-input-inline"><input type="text" name="come_account" placeholder="来源于会员账号" class="layui-input" /></div></div>
                <div class="layui-inline"><div class="layui-input-inline"><input type="text" name="note" placeholder="关键字查询" class="layui-input" /></div></div>
                <div class="layui-inline">
                    <div class="layui-input-inline">
                        <select name="bonus_id" class="layui-input">
                            <option value="">--奖金类型--</option>
                            <?php if(is_array($bonusSingle)): foreach($bonusSingle as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                        </select>
                    </div>
                </div>
                ...
                <button type="button" class="clearCache layui-btn  layui-btn-danger pull-right" onclick="emptyEp();"><i class="layui-icon">&#xe60b;</i> 清空奖金记录</button>
...
<script>
    ...
    function ajax_get_table(tab, page){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo U('Bonus/bonusLog');?>?p=" + page,
            data: $('#' + tab).serialize(),
    ...
    function emptyEp(){
        layer.confirm("是否确认清空数据，执行之后将不能撤销", {title:'温馨提示', icon:3}, function(){
            ...
            var url = "<?php echo U('Bonus/emptyBonusLog');?>";
