    ...
    <fieldset class="layui-elem-field">
        <div class="test-table-reload-btn" style="margin-top: 10px;margin-left:15px;">
            <form action="<?php echo U('Excel/exportUserActionLog');?>" class='layui-form' id="search-form2">
                ...
                <div class="layui-inline">
                    <input type="text" name="title" placeholder="关键词" class="layui-input" />
                </div>
                <div class="layui-inline" style='width:120px;'>
                    <select name="type">
                        <option value="">--分类属性--</option>
                        <?php if(is_array($messageType)): foreach($messageType as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
...
<script>
    ...
    function ajax_get_table(tab, page){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo U('Message/messageIndex');?>?p=" + page,
            data: $('#' + tab).serialize(),
