<?php if (!defined('THINK_PATH')) exit();?><table class="layui-table layui-form">
    <thead>
        <tr>
            ...
            <?php if(is_array($levelList)): $i = 0; $__LIST__ = $levelList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vl): $mod = ($i % 2 );++$i;?><th><a href="javascript:sort('level_<?php echo ($vl["level_id"]); ?>');"><?php echo ($vl["name_cn"]); ?></a></th><?php endforeach; endif; else: echo "" ;endif; ?>
            ...
            <?php if(is_array($moneylist)): $i = 0; $__LIST__ = $moneylist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vm): $mod = ($i % 2 );++$i;?><th><a href="javascript:sort('money_<?php echo ($vm["money_id"]); ?>');"><?php echo ($vm["name_cn"]); ?></a></th><?php endforeach; endif; else: echo "" ;endif; ?>
            <th>操作</th>
        </tr>
    </thead>
    <tbody>
    <?php if(count($list) == 0): ?><tr align="center">
        ...
        <?php if(is_array($list)): foreach($list as $k=>$v): ?><tr>
                ...
                <?php if(is_array($levelList)): $i = 0; $__LIST__ = $levelList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vla): $mod = ($i % 2 );++$i;?><td><?php echo $v['level_'.$vla['level_id']]; ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
                ...
                <?php if(is_array($moneylist)): $i = 0; $__LIST__ = $moneylist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vma): $mod = ($i % 2 );++$i;?><td><?php echo $v['money_'.$vma['money_id']]; ?></td><?php endforeach; endif; else: echo "" ;endif; ?>
