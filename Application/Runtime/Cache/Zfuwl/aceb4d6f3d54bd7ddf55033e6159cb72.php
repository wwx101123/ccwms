...
<div class="admin-main">
    <fieldset class="layui-elem-field">
        ...
        <form action="<?php echo U('Excel/exportCarryLog');?>" id="search-form2">
            ...
            <div class="layui-inline"><div class="layui-input-inline"><input type="text" name="note" placeholder="关键字" class="layui-input" /></div></div>
            <div class="layui-inline">
                <div class="layui-input-inline">
                    <select name="mid" class="layui-input">
                        <option value="">--钱包选择--</option>
                        <?php if(is_array($moneyCarryInfo)): foreach($moneyCarryInfo as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="layui-inline">
                <div class="layui-input-inline">
                    <select name="statu" class="layui-input">
                        ...
                        <?php if(is_array($moneyCarryLogStatu)): foreach($moneyCarryLogStatu as $k=>$v): ?><option value="<?php echo ($k); ?>"><?php echo ($v); ?></option><?php endforeach; endif; ?>
                    </select>
                </div>
            </div>
            <div class="layui-inline"><div class="layui-input-inline"><input type="text" name="time" placeholder="开始日" id="time" class="layui-input" /></div></div>
            ...
            <a href="<?php echo U('Carry/addCarrylist');?>"><button type="button" class="layui-btn layui-btn-normal" style="float:right;"><i class="layui-icon">&#xe61f;</i> 批量提现</button></a>
            <button type="button" class="clearCache layui-btn  layui-btn-danger pull-right" onclick="emptyEp();"><i class="layui-icon">&#xe60b;</i> 清空提现记录</button>
...
<script>
    ...
    function ajax_get_table(tab, page){
        ...
        $.ajax({
            type: "POST",
            url: "<?php echo U('Carry/carryLog');?>?p=" + page,
            data: $('#' + tab).serialize(),
    ...
    function emptyEp(){
        layer.confirm("是否确认清空数据，执行之后将不能撤销", {title:'温馨提示', icon:3}, function(){
            ...
            var url = "<?php echo U('Carry/emptyCarryLog');?>";
