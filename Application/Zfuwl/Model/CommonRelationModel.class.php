...
class CommonRelationModel extends RelationModel {
    ...
    public function selectAllListAjaxRelation($where, $order = array(), $pageNum = PAGE_LIMIT) {
        $count = $this->dataCount($where);
        $page = ajaxGetPage($count, $pageNum);
        $show = $page->show();
        $list = $this->where($where)->Relation(true)->order($order)->limit($page->firstRow . ',' . $page->listRows)->select();
    ...
    public function selectAllListRelation($where, $order = array(), $pageNum = PAGE_LIMIT) {
        $count = $this->dataCount($where);
        $page = getPage($count, $pageNum);
//        $page = ajaxGetPage($count, $pageNum);
        $show = $page->show();
